...
if [ "$#" -eq 1 ]; then
  ...
  if [[ $j < 1 ]]; then
    cd ${NYRAH_DIRECTORY}/${SERVER_NAME}

    yes | cp ${OUTPUT_DIRECTORY}/Nyrah ${NYRAH_DIRECTORY}/${SERVER_NAME}
    yes | cp ${CLOUD_DIRECTORY}/scripts/server/nyrah/settings.json ${NYRAH_DIRECTORY}/${SERVER_NAME}
    
    echo -e "${COLOR_GREEN}Ligando o ${SERVER_NAME} em $(pwd)...${COLOR_RESET}"
    screen -dmS ${SERVER_NAME} ./Nyrah
  else
